## Tóm Tắt Dự Án: NextJs\_SpringBoot

## Tổng Quan

Đây là ứng dụng full-stack sử dụng **Spring Boot** cho backend và **Next.js** cho frontend, cung cấp nền tảng quản lý và giám sát thông minh.

### Các Thành Phần Chính

| Thành phần | Công nghệ | Mô tả |
|------------|-----------|-------|
| **Backend** | Spring Boot 4.0.5, Java 21 | REST API, WebSocket, GraphQL, xác thực JWT |
| **Frontend** | Next.js 16.2.2, React 19 | UI hiện đại, real-time chat, telemetry dashboard |
| **Database** | SQLite + JPA/Hibernate | Lưu trữ users, permissions, vehicle telemetry |
| **Real-time** | WebSocket + STOMP | Chat thời gian thực, status updates |

### Tính Năng Nổi Bật

1. **Xác thực bảo mật** - JWT tokens với refresh token rotation, mã hóa BCrypt
2. **Chat real-time** - WebSocket STOMP protocol, chat công khai và riêng tư
3. **Giám sát telemetry** - 55+ điểm dữ liệu xe, GraphQL API, infinite scroll
4. **Phân quyền linh hoạt** - 3 vai trò (admin, member, guest) với kiểm soát truy cập RBAC
5. **Kiến trúc hiện đại** - Stateless authentication, hybrid REST + GraphQL, responsive UI
...
*   **refresh\_tokens**: Quản lý refresh token JWT
*   **behicle\_telemetry\_master**: Dữ liệu telemetry xe (55+ cột)
